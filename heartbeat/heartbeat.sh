...
echo /usr/bin/aws s3 cp /var/tmp/solarshed.heartbeat.txt s3://${BUCKET}/heartbeat.txt
/usr/bin/aws s3 cp /var/tmp/solarshed.heartbeat.txt s3://${BUCKET}/heartbeat.txt
echo $?
